...
export default {
	...
	async execute(interaction: ChatInputCommandInteraction) {
		console.log("commands/feeds/nouns-fork: Adding new feed.", {
		...
		const eventResults = [];
		if (event === "all") {
			const results = await FeedConfig.registerAllProjectFeeds(interaction.guildId!, channel.id, "NounsFork");
		...
		console.log("commands/feeds/nouns-fork: Finished adding new feed.", {
			userId: interaction.user.id,
			guildId: interaction.guildId,
			channelId: interaction.channelId
		});
