...
export default {
	...
	async execute(interaction: ChatInputCommandInteraction) {
		console.log("commands/feeds/federation: Adding new feed.", {
		...
		const eventResults = [];
		if (event === "all") {
			const results = await FeedConfig.registerAllProjectFeeds(interaction.guildId!, channel.id, "Federation");
		...
		console.log("commands/feeds/federation: Finished adding new feed.", {
			userId: interaction.user.id,
			guildId: interaction.guildId,
			channelId: interaction.channelId
		});
