...
export default {
	...
	async execute(interaction: ChatInputCommandInteraction) {
		console.log("commands/feeds/propdates: Adding new feed.", {
		...
		const eventResults = [];
		if (event === "all") {
			const results = await FeedConfig.registerAllProjectFeeds(interaction.guildId!, channel.id, "Propdates");
		...
		console.log("commands/feeds/propdates: Finished adding new feed.", {
			userId: interaction.user.id,
			guildId: interaction.guildId,
			channelId: interaction.channelId
		});
