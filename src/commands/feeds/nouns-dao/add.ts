...
export default {
	...
	async execute(interaction: ChatInputCommandInteraction) {
		console.log("commands/feeds/nouns-dao: Adding new feed.", {
		...
		const eventResults = [];
		if (event === "all") {
			const results = await FeedConfig.registerAllProjectFeeds(interaction.guildId!, channel.id, "Nouns");
		...
		console.log("commands/feeds/nouns-dao: Finished adding new feed.", {
			userId: interaction.user.id,
			guildId: interaction.guildId,
			channelId: interaction.channelId
		});
